		...
		for(CtInvocation<?> invoc : invocationList){
			...
			while (iteratorMethodsNames.hasNext()) {
				...
				if(invoc.getExecutable().equals(pair.getKey())){
					...
					System.out.println(invoc.getExecutable().getParameters());
					//add cast if null
					int i = 0;
					for( CtExpression<?> arg : invoc.getArguments()){
						/*System.out.println("ARG = "+arg);
						System.out.println("TYPE ="+arg.getType().getQualifiedName());
						System.out.println(arg.getType().getQualifiedName().equals(CtTypeReference.NULL_TYPE_NAME));
						System.out.println(CtTypeReference.NULL_TYPE_NAME);
						System.out.println(arg.getTypeCasts());*/
						if(arg.getType() != null && arg.getType().getQualifiedName() != null){
							if(arg.getType().getQualifiedName().equals(CtTypeReference.NULL_TYPE_NAME)){
								System.out.println(".... cast argument "+i+" in "+invoc.getExecutable().getParameters().get(i));
								List<CtTypeReference<?>> casts = new ArrayList<CtTypeReference<?>>();
								casts.add(invoc.getExecutable().getParameters().get(i));
								arg.setTypeCasts(casts);
							}
						}
						i++;
					}
